...
import { UserProfilesRepository, UserWithMusicPreferences } from './userProfiles.repository';
...
import { MatchWithSimilarity, calculateAudioFeatureCompatibilityScore, calculateCompatibilityScore, calculateDesirability, calculatePriorityScore } from './utils';
    ...
    async getPossibleMatches(params: {
        ...
        const potentialMatches = await this.userRepository.getAllUserProfiles({
            where: {
              ...
              ...(currentUser.genderPreference != 'ALL' ? { gender: currentUser.genderPreference } : {}),
              NOT: {
        ...
        const filteredMatches = potentialMatches.filter((_, index) => newMatches[index]);
        ...
        const matches = filteredMatches.map(async (potentialMatch) => {            
            ...
            const personalityPreferenceScore = currentUser.samePersonalityPreference && currentUser.personalityType === potentialMatch.personalityType ? Math.random() : 0
            ...
            const allSuccessfulMatches: Array<MatchWithSimilarity> = acceptedUserMatches.map((match) => ({
                id: match.id,
                matchScore: match.matchScore,
            }));

            const desirabilityScore = allSuccessfulMatches.length === 0 ? 0 : calculateDesirability(allSuccessfulMatches, totalMatches.length);
            ...
            const potentialMatchPending = await this.matchesRepository.getMatches({
                where: {
                  OR: [
                    { user1Id: potentialMatch.id, user2Id: currentUser.id, matchStatus: 'PENDING' },
                    { user1Id: currentUser.id, user2Id: potentialMatch.id, matchStatus: 'PENDING' },
                  ],
                }
            });
            ...
            const finalScore = (personalityPreferenceScore * Math.random()) + (audioCompatibilityScore) + compatibilityScore + desirabilityScore + matchPriority + joinDatePriority;
        ...
        const userProfilePromises: Promise<UserWithMusicPreferences>[] = finalArray!.map(async (current) => {
            return (await this.userRepository.getUserProfile({
