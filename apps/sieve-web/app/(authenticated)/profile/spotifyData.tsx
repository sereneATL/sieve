...
export const getSpotifyData = async (params: {
  ...
  const res = await fetch(`${spotifyBaseUrl}${params.path}`, {
    headers: {
...
export const aggregateAudioFeatures = async (
  ...
  const promises = topTracks.map(
    (trackId): Promise<SpotifyAudioFeatures> =>
...
export const compileSpotifyTracksData = async (
  token: string,
): Promise<{ trackNames: string[]; id: string[] }> => {
...
export const compileSpotifyArtistsData = async (
  token: string,
): Promise<{ names: string[]; topGenres: string[] }> => {
  const data = await getSpotifyData({
    path: "/me/top/artists?limit=49",
    token,
  });
...
export const sumNumericValues = (
  data: SpotifyAudioFeatures[],
